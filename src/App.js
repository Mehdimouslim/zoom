  ...
  return (
    <div style={{paddingLeft:40}}>
      ...
      <div align="center">
        ...
        :<button className='button button1' onClick={()=>handleEnregistrer()}>Enregister</button>}
        <button className='button button1' onClick={()=>handleClear()}>Clear</button>
      </div>
      
      <div>
        <label>Filtrer par ville</label>
        <select className='col-75' value={villeFilter} onChange={(e)=>setVilleFilter(e.target.value)}>
          {villes.map((ville, i)=><option key={i} value={ville.id}>{ville.nom}</option>)}
        </select>
        <button className='button button8' onClick={()=>handleFilter()}>Filtrer</button>
        <button className='button button9' onClick={()=>handleFilterClear()}>Clear</button>
      </div>
      <table>
        <thead>
          <tr>
          ...
          {listeusersmap.map((user,index)=> {
            ...
            return (
              <tr key={index}>
                ...
                <td>{ville.nom}</td>
                <td>
                  <button className='button button8' onClick={()=>handleRemplirForm(user.id)}>Modifier</button>
                  <button className='button button9' onClick={()=>handleDelete(user.id)}>Supprimer</button>
                </td>
              </tr>
            )
          })}
